...
export default function QRCodeView({ size = 256 }: { size?: number }) {
  ...
  const qrText = `https://oleo-soft.com/index.html?id=${equipment.equipmentsId}&name=${equipment.name}&number=${equipment.number}`;
  ...
  const descargarQR = () => {
    ...
    img.onload = () => {
      ...
      downloadLink.download = `qr-code-${equipment.number}.png`;
